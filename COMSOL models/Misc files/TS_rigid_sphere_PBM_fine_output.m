function out = model
%
% TS_rigid_sphere_PBM_fine_output.m
%
% Model exported on Sep 3 2026, 10:41 by COMSOL 6.4.0.429.
...
model.modelPath('/ocean/projects/bio240005p/vchhaya');

model.label('TS_rigid_sphere_PBM_fine.mph');

model.param.set('R_scat', '2.5 [cm]', 'Radius of spherical scatterer');
model.param.set('R_dom', '4.5 [cm]', 'Radius of medium domain');
...
model.study('std1').feature('freq').set('plist', '10^(range(log10(50),1/40,log10(90000)))');
model.study('std1').run;

model.label('TS_rigid_sphere_PBM_fine_output.mph');
...
model.result('pg1').feature('ptgr1').set('expr', 'acpr.efc1.pext');
model.result('pg1').feature('ptgr1').set('descr', 'Exterior-field pressure');
model.result('pg1').feature('ptgr1').set('expr', '20*log10(abs(acpr.efc1.pext))');
model.result('pg1').run;
...
model.result.export('tbl1').set('filename', 'C:\Users\Vaibhav Chhaya\Echospace\Benchmarking\Results\rigid_sphere_COMSOL.csv');
model.result('pg1').run;
model.result('pg1').run;
model.result('pg1').run;
